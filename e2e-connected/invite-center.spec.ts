...
test('만료된 초대 수락 오류는 목록 새로고침 뒤에도 사용자에게 남는다', async ({ page }) => {
  ...
  let initialInviteReads = 0
  await page.route('**/rest/v1/rpc/get_game_invites', async route => {
    initialInviteReads += 1
    await new Promise(resolve => setTimeout(resolve, 1_200))
    await route.continue()
  })
  await login(page, accounts[1].email, password)
  await expect(page.getByRole('button', { name: '게임 초대 1개' })).toBeVisible()
  expect(initialInviteReads).toBe(1)
  await page.unroute('**/rest/v1/rpc/get_game_invites')
