...
export default function Account() {
  ...
  return <div className="content-page account-page"><PageHeader eyebrow="MY ACCOUNT" title="내 계정을 관리해요." description="프로필, 보안, 로그인 상태를 한곳에서 확인하세요." />
    ...
    <div className="account-grid">
      <form className="form-card account-card" onSubmit={saveProfile}><h2><UserRound /> 프로필</h2>{profile ? <><div className="account-avatar" aria-hidden="true">{nickname.slice(0, 1) || '?'}</div><label><span>닉네임</span><input value={nickname} onChange={event => setNickname(event.target.value.slice(0, 12))} minLength={2} maxLength={12} required /></label><label><span>아바타 스타일</span><div className="account-inline"><input value={avatarSeed} onChange={event => setAvatarSeed(event.target.value.slice(0, 32))} /><button type="button" aria-label="아바타 무작위 변경" onClick={() => setAvatarSeed(crypto.randomUUID().slice(0, 12))}><RefreshCw /></button></div></label><div className="account-readonly"><span>친구 태그</span><strong>{profile.friendTag}</strong><button type="button" aria-label="친구 태그 복사" onClick={() => void navigator.clipboard?.writeText(profile.friendTag)}><Copy /></button></div><button className="primary-button full-button" disabled={busy === 'profile'}>{busy === 'profile' ? '저장 중...' : <><Check /> 프로필 저장</>}</button></> : <p role="status">프로필을 불러오는 중...</p>}</form>
