...
export default function Account() {
  ...
  const [activeSessionAttempt, setActiveSessionAttempt] = useState(0)
  const activeSessionRequestRef = useRef<{ attempt: number; promise: ReturnType<typeof findMyActiveSession> } | null>(null)
  const requestActiveSession = useCallback((attempt: number) => {
    if (activeSessionRequestRef.current?.attempt !== attempt) {
      activeSessionRequestRef.current = { attempt, promise: findMyActiveSession() }
    }
    return activeSessionRequestRef.current.promise
  }, [])
  ...
  useEffect(() => {
    ...
    void getAccountProfile()
      .then(value => { if (active) { setProfile(value); setNickname(value.nickname); setAvatarSeed(value.avatarSeed); setError('') } })
      .catch(() => { if (active) { setProfile(null); setError('프로필을 불러오지 못했어요. 잠시 후 다시 시도해 주세요.') } })
      .finally(() => { if (active) setProfileLoading(false) })
    return () => { active = false }
  }, [profileLoadAttempt])
  useEffect(() => {
    let active = true
    setActiveSessionLoading(true); setActiveSessionError(false)
    void requestActiveSession(activeSessionAttempt)
      .then(session => { if (active) setActiveSession(session) })
      .catch(() => { if (active) { setActiveSession(null); setActiveSessionError(true) } })
      .finally(() => { if (active) setActiveSessionLoading(false) })
    return () => { active = false }
  }, [activeSessionAttempt, requestActiveSession])
  ...
  return <div className="content-page account-page"><PageHeader eyebrow="MY ACCOUNT" title="내 계정을 관리해요." description="프로필, 보안, 로그인 상태를 한곳에서 확인하세요." />
    ...
    <div className="account-grid">
      <form className="form-card account-card" onSubmit={saveProfile}><h2><UserRound /> 프로필</h2>{profile ? <><div className="account-avatar" aria-hidden="true">{nickname.slice(0, 1) || '?'}</div><label><span>닉네임</span><input value={nickname} onChange={event => setNickname(event.target.value.slice(0, 12))} minLength={2} maxLength={12} required /></label><label><span>아바타 스타일</span><div className="account-inline"><input value={avatarSeed} onChange={event => setAvatarSeed(event.target.value.slice(0, 32))} /><button type="button" aria-label="아바타 무작위 변경" onClick={() => setAvatarSeed(createShortId())}><RefreshCw /></button></div></label><div className="account-readonly"><span>친구 태그</span><strong>{profile.friendTag}</strong><button type="button" aria-label="친구 태그 복사" onClick={() => void copyFriendTag()}><Copy /></button></div><button className="primary-button full-button" disabled={busy === 'profile'}>{busy === 'profile' ? '저장 중...' : <><Check /> 프로필 저장</>}</button></> : profileLoading ? <p role="status">프로필을 불러오는 중...</p> : <div className="admin-empty"><strong>프로필을 표시할 수 없어요.</strong><p>연결을 확인한 뒤 다시 시도해 주세요.</p><button type="button" className="secondary-button" onClick={() => setProfileLoadAttempt(value => value + 1)}>프로필 다시 불러오기</button></div>}</form>
      <section className="form-card account-card"><h2><ShieldCheck /> 로그인 정보</h2><div className="account-identity">{user?.email ? <AtSign /> : <Phone />}<span><strong>{user?.email ?? user?.phone ?? '연결된 계정'}</strong><small>{user?.email ? user.emailConfirmed ? '이메일 인증 완료' : '이메일 인증 대기' : user?.phoneConfirmed ? '전화번호 인증 완료' : '전화번호 인증 대기'}</small></span></div>{activeSessionLoading && <p className="account-session-state" role="status">진행 중인 게임을 확인하고 있어요.</p>}{activeSessionError && <div className="account-session-state is-error"><span>진행 중인 게임을 확인하지 못했어요.</span><button type="button" onClick={() => setActiveSessionAttempt(value => value + 1)}>다시 확인</button></div>}{activeSession && <button className="secondary-button full-button" onClick={() => navigate(activeSession.type === 'game' ? `/game?game=${encodeURIComponent(activeSession.gameId)}` : `/room/${encodeURIComponent(activeSession.roomId)}`)}>진행 중인 {activeSession.type === 'game' ? '게임' : '대기방'}으로 이동</button>}<button className="secondary-button full-button" onClick={() => void signOut()}><LogOut /> 이 기기에서 로그아웃</button><button className="text-button full-button" onClick={() => void signOutAll()}><LogOut /> 모든 기기에서 로그아웃</button></section>
