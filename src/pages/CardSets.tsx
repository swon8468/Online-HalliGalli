...
export default function CardSets() {
  ...
  useEffect(() => {
    setModal(null); setSource(null); setDeleteTarget(null); setError(''); setMessage(''); setSpaceId(requestedSpace ?? '')
    void refresh()
  }, [refresh, requestedSpace])
  ...
  const submit = async (event: FormEvent) => {
    ...
    const token = Symbol(modal ?? 'create')
    ...
    try {
      ...
      if (activeScopeRef.current === scope) setMessage(modal === 'clone' ? '카드 세트를 복제했어요.' : '초안 카드 세트를 만들었어요.')
    } catch (cause) {
      if (activeScopeRef.current === scope) setError(getErrorMessage(cause, '카드 세트를 만들지 못했어요.'))
    } finally {
      if (actionTokenRef.current === token) { actionTokenRef.current = null; setBusy(false) }
    }
  }
  const remove = async () => {
    if (!deleteTarget || actionTokenRef.current) return
    const scope = scopeKey
    ...
    try {
      ...
      if (activeScopeRef.current !== scope) return
      setDeleteTarget(null); await refresh(true)
      if (activeScopeRef.current === scope) setMessage('카드 세트를 삭제했어요.')
    } catch (cause) {
      if (activeScopeRef.current === scope) setError(getErrorMessage(cause, '삭제하지 못했어요.'))
    } finally {
      if (actionTokenRef.current === token) { actionTokenRef.current = null; setBusy(false) }
    }
  }
