...
export default function SpaceAdmin() {
  ...
  useEffect(() => {
    setSnapshot(null); setTab('overview'); setError(''); setMessage(''); setMemberOpen(false); setConfirmAction(null); setCreatedCredentials([]); setCsvRows([])
    void refresh()
  }, [refresh])
  const run = async (key: string, action: () => Promise<unknown>, success: string) => {
    ...
    try {
      await action()
      if (activeSlugRef.current !== scope) return
      await refresh(true, true)
      if (activeSlugRef.current === scope) setMessage(success)
    } catch (cause) {
      if (activeSlugRef.current === scope) setError(getErrorMessage(cause, '작업을 완료하지 못했어요.'))
    } finally {
  ...
  const submitMember = (event: FormEvent) => {
    ...
    if (memberMode === 'csv') {
      ...
      void run('member', async () => {
        const result = await bulkCreateSpaceAccounts(snapshot.space.id, csvRows, 'CSV 멤버 일괄 등록')
        if (activeSlugRef.current !== scope) return
        setCreatedCredentials(result.accounts.map(item => ({ email: item.email, password: item.password }))); setMemberOpen(false); setCsvRows([])
      }, `${csvRows.length}명 CSV 등록 요청을 처리했어요.`); return
    }
    const scope = slug
    void run('member', async () => {
      if (memberMode === 'existing') await addExistingSpaceMember(snapshot.space.id, account, '기존 계정 스페이스 초대')
        ...
        if (activeSlugRef.current === scope && result.account.password) setCreatedCredentials([{ email: result.account.email, password: result.account.password }])
      }
      if (activeSlugRef.current !== scope) return
  ...
  const saveSettings = (event: FormEvent) => { event.preventDefault(); if (!snapshot) return; const scope = slug; void run('settings', async () => { await updateSpace(snapshot.space.id, { name, slug: spaceSlug, description, joinEnabled, reason: '스페이스 설정 변경' }); if (activeSlugRef.current === scope && spaceSlug !== scope) navigate(`/spaces/${encodeURIComponent(spaceSlug)}/admin`, { replace: true }) }, '스페이스 설정을 저장했어요.') }
  ...
  return <div className="content-page space-admin-page"><PageHeader eyebrow="SPACE CONTROL" title={`${snapshot.space.name} 관리`} description="멤버, 계정, 방, 전용 카드와 행사 운영 상태를 관리하세요." />
    ...
    {tab === 'overview' && <><section className="space-metrics"><article><span>멤버</span><strong>{snapshot.members.length}</strong><small>관리자 {snapshot.members.filter(member => ['owner', 'manager'].includes(member.role)).length}명</small></article><article><span>활성 방</span><strong>{activeRooms}</strong><small>전체 {snapshot.rooms.length}개</small></article><article><span>완료 게임</span><strong>{completedGames}</strong><small>기록 {snapshot.games.length}개</small></article><article><span>카드 세트</span><strong>{snapshot.cardSets.length}</strong><small>게시 {snapshot.cardSets.filter(set => set.status === 'published').length}개</small></article></section><section className="form-card space-invite-card"><div><Link2 /><span><strong>가입 링크</strong><small>{snapshot.space.joinEnabled ? '링크를 가진 사용자가 직접 가입할 수 있습니다.' : '현재 가입 코드 사용이 중지되었습니다.'}</small><small>허용 이메일: {snapshot.space.allowedEmailDomain ?? '기존 스페이스 · 제한 없음'}</small></span></div><code>{snapshot.space.joinCode}</code><div><button className="secondary-button" onClick={() => void copyWithFeedback(inviteUrl, '가입 링크를 복사했어요.', '가입 링크를 복사하지 못했어요. 화면에서 직접 선택해 주세요.')}><Copy /> 링크 복사</button><button className="secondary-button" disabled={Boolean(busy)} onClick={() => void run('rotate', async () => { await rotateSpaceCode(snapshot.space.id, '가입 코드 재발급') }, '새 가입 코드를 발급했어요.')}><RefreshCw /> {busy === 'rotate' ? '발급 중...' : '코드 재발급'}</button></div></section></>}
    {tab === 'members' && <section className="admin-panel space-admin-panel"><header><div><h2>멤버와 계정</h2><p>기존 계정 초대, 개별 계정 생성, CSV 일괄 생성을 지원합니다.</p></div>{snapshot.actor.canManage && <button className="admin-primary" disabled={Boolean(busy)} onClick={() => setMemberOpen(true)}><UserPlus /> 멤버 추가</button>}</header><div className="space-member-table"><div className="space-member-head"><span>사용자</span><span>식별 번호</span><span>역할</span><span>상태</span><span>관리</span></div>{snapshot.members.map(member => <div className="space-member-row" key={member.userId}><span><i className="avatar avatar--2">{member.nickname[0]}</i><span><strong>{member.nickname}</strong><small>{member.email ?? member.phone ?? member.friendTag}</small></span></span><span>{member.externalId || '-'}</span><select aria-label={`${member.nickname} 역할`} value={member.role} disabled={Boolean(busy) || !snapshot.actor.canManage || member.role === 'owner'} onChange={event => void run(`role-${member.userId}`, () => updateSpaceMember(snapshot.space.id, member.userId, event.target.value as 'member' | 'manager', '스페이스 역할 변경'), `${member.nickname}님의 역할을 변경했어요.`)}><option value="member">멤버</option><option value="manager">관리자</option>{member.role === 'owner' && <option value="owner">소유자</option>}</select><span className={member.deleted || member.suspended ? 'status-badge is-banned' : 'status-badge'}>{member.deleted ? '탈퇴' : member.suspended ? '정지' : '정상'}</span><button aria-label={`${member.nickname} 삭제`} disabled={Boolean(busy) || !snapshot.actor.canManage || member.role === 'owner'} onClick={() => setConfirmAction({ type: 'remove-member', member })}><Trash2 /></button></div>)}</div>{createdCredentials.length > 0 && <div className="space-credentials"><strong>이번에 생성된 임시 로그인 정보</strong><p>비밀번호는 이 화면을 닫으면 다시 확인할 수 없습니다.</p>{createdCredentials.map(item => <code key={item.email}>{item.email} · {item.password ?? '기존 계정'}</code>)}<button onClick={() => void copyWithFeedback(createdCredentials.map(item => `${item.email},${item.password ?? ''}`).join('\n'), '임시 로그인 정보를 복사했어요.', '임시 로그인 정보를 복사하지 못했어요. 화면에서 직접 선택해 주세요.')}><Copy /> 복사</button></div>}</section>}
    ...
    {tab === 'settings' && <form className="form-card space-settings" onSubmit={saveSettings}><h2><Building2 /> 기본 설정</h2><label><span>스페이스 이름</span><input value={name} onChange={event => setName(event.target.value)} minLength={2} maxLength={80} required disabled={Boolean(busy)} /></label><label><span>Slug</span><input value={spaceSlug} onChange={event => setSpaceSlug(event.target.value.toLowerCase().replace(/[^a-z0-9-]/g, '').slice(0, 49))} minLength={3} required disabled={Boolean(busy)} /></label><label><span>설명</span><textarea rows={3} value={description} onChange={event => setDescription(event.target.value)} disabled={Boolean(busy)} /></label><label className="space-toggle"><input type="checkbox" checked={joinEnabled} onChange={event => setJoinEnabled(event.target.checked)} disabled={Boolean(busy)} /><span>가입 코드 활성화</span></label><button className="primary-button full-button" disabled={Boolean(busy)}><Save /> {busy === 'settings' ? '저장 중...' : '설정 저장'}</button><button type="button" className="danger-text-button full-button" disabled={Boolean(busy) || snapshot.space.status === 'archived'} onClick={() => setConfirmAction({ type: 'archive-space' })}><Archive /> 스페이스 비활성화</button></form>}
    ...
    {memberOpen && <div className="admin-modal-backdrop" onMouseDown={event => { if (event.target === event.currentTarget && !busy) setMemberOpen(false) }}><form className="admin-modal space-member-modal" role="dialog" aria-modal="true" aria-labelledby="space-member-title" onSubmit={submitMember}><span><UsersRound /></span><h2 id="space-member-title">멤버 추가</h2><div className="space-member-modes"><button type="button" disabled={Boolean(busy)} className={memberMode === 'existing' ? 'is-active' : ''} onClick={() => setMemberMode('existing')}>기존 계정</button><button type="button" disabled={Boolean(busy)} className={memberMode === 'create' ? 'is-active' : ''} onClick={() => setMemberMode('create')}>계정 생성</button><button type="button" disabled={Boolean(busy)} className={memberMode === 'csv' ? 'is-active' : ''} onClick={() => setMemberMode('csv')}>CSV</button></div>{memberMode === 'csv' ? <><label className="space-csv-upload"><FileUp /> CSV 파일 선택<input type="file" accept=".csv,text/csv" disabled={Boolean(busy)} onChange={event => void parseCsv(event)} /></label><p>헤더: email,nickname,external_id,role,password · 최대 100명</p><strong>{csvRows.length}개 행 준비됨</strong></> : <><label>이메일<input type="email" value={account.email} onChange={event => setAccount({ ...account, email: event.target.value })} required disabled={Boolean(busy)} /></label>{memberMode === 'create' && <><label>닉네임<input value={account.nickname} onChange={event => setAccount({ ...account, nickname: event.target.value })} minLength={2} maxLength={12} required disabled={Boolean(busy)} /></label><label>임시 비밀번호<input type="password" value={account.password} onChange={event => setAccount({ ...account, password: event.target.value })} placeholder="비워두면 자동 생성" disabled={Boolean(busy)} /></label></>}<label>단체 내 식별 번호<input value={account.externalId} onChange={event => setAccount({ ...account, externalId: event.target.value })} disabled={Boolean(busy)} /></label><label>역할<select value={account.role} onChange={event => setAccount({ ...account, role: event.target.value as 'member' | 'manager' })} disabled={Boolean(busy)}><option value="member">멤버</option><option value="manager">관리자</option></select></label></>}<div><button type="button" className="secondary-button" disabled={Boolean(busy)} onClick={() => setMemberOpen(false)}>취소</button><button className="full-button" disabled={Boolean(busy)}>{busy === 'member' ? '처리 중...' : '추가'}</button></div></form></div>}
  </div>
}
