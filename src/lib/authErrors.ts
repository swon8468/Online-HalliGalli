...
export function translateAuthError(error: unknown, fallback = '계정 정보를 확인해 주세요.') {
  ...
  if (normalized.includes('error sending') && normalized.includes('email')) return '인증 메일을 보내지 못했어요. 잠시 후 다시 시도해 주세요.'
  if (normalized.includes('smtp')) return '인증 메일 발송 설정을 확인할 수 없어요. 관리자에게 문의해 주세요.'
