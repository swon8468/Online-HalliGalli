...
test('데모 회원가입의 중복 확인과 비밀번호 확인 흐름이 동작한다', async ({ page }) => {
  ...
  await expect(page.locator('#auth-identifier-error')).toHaveText('올바른 이메일 주소를 입력해 주세요.')
...
test('렌더링 오류는 제한된 웹뷰에서도 복구 화면과 홈 이동을 제공한다', async ({ page }) => {
  await page.addInitScript(() => {
    Object.defineProperty(window.crypto, 'randomUUID', { configurable: true, value: undefined })
  })
  await page.setViewportSize({ width: 320, height: 568 })
  await page.goto('/?_testCrash=1')

  await expect(page.getByRole('heading', { name: '화면을 불러오지 못했어요.' })).toBeVisible()
  await expect(page.locator('.fatal-error-page code')).toHaveText(/^UI-[A-Z0-9]+-[A-Z0-9]{6}$/)
  expect(await page.evaluate(() => document.documentElement.scrollWidth - document.documentElement.clientWidth)).toBe(0)
...
test('요구 화면 크기 전체에서 게임은 가로 스크롤 없이 핵심 조작을 표시한다', async ({ page }, testInfo) => {
  ...
  for (const viewport of [
    ...
    const layout = await page.evaluate(() => ({
      ...
      deckTarget: (() => { const rect = document.querySelector('.player-deck')?.getBoundingClientRect(); return rect ? [rect.width, rect.height] : [0, 0] })(),
      bellTarget: (() => { const rect = document.querySelector('.arena-bell')?.getBoundingClientRect(); return rect ? [rect.width, rect.height] : [0, 0] })(),
    }))
    expect(layout.horizontalOverflow, `${viewport.width}px 가로 오버플로`).toBe(0)
    expect(Math.min(...layout.deckTarget), `${viewport.width}px 카드 터치 영역`).toBeGreaterThanOrEqual(44)
    expect(Math.min(...layout.bellTarget), `${viewport.width}px 종 터치 영역`).toBeGreaterThanOrEqual(44)
  }
  expect(errors).toEqual([])
})

test('빠른 중복 탭과 화면 회전은 카드 한 장만 공개하고 상태를 유지한다', async ({ page }) => {
  const errors = watchRuntimeErrors(page)
  await page.setViewportSize({ width: 390, height: 844 })
  await page.goto('/game?mode=bot&difficulty=easy')
  const deck = page.getByRole('button', { name: /28 눌러서 뒤집기/ })
  await deck.evaluate(button => { button.click(); button.click() })
  await expect(page.getByRole('region', { name: '내 플레이 영역' })).toContainText('27장')
  const cardId = await page.locator('[data-card-id]').first().getAttribute('data-card-id')
...
test('PWA 설치 취소·오프라인 복구·업데이트 안내가 다시 사용할 수 있는 상태로 끝난다', async ({ page }, testInfo) => {
  ...
  await expect(page.getByText('다시 연결됐어요.')).toHaveCount(0)

  await page.evaluate(() => {
    window.__halliGalliUpdateSW = async () => { throw new Error('offline update failure') }
    window.dispatchEvent(new Event('halli-galli:pwa-update'))
  })
  await page.getByRole('button', { name: '지금 업데이트' }).click()
  await expect(page.getByText('업데이트를 적용하지 못했어요. 연결을 확인한 뒤 다시 시도해 주세요.')).toBeVisible()
